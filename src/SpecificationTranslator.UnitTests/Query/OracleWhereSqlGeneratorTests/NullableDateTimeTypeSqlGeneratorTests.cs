...
{
    [TestFixture]
    public class NullableDateTimeTypeSqlGeneratorTests : DateTimeTypeSqlGeneratorTestsBase, ICompareTests, IInTests
    {
        private DateTime _expectedDateTime;
        private string _expectedOracleDateTime;

        public NullableDateTimeTypeSqlGeneratorTests()
        {
        ...
        {
            var specification = new AnonymousSpecification<UserStub>(v => v.ModifyDate == _expectedDateTime);
            string actualSql = GenerateSql(specification);

            Assert.AreEqual($"(ModifyDate = {_expectedOracleDateTime})", actualSql);
        }
        ...
        public void Generate_GenerateFromIn_ShouldEqualsSqlResult()
        {
            ...
            var specification = new AnonymousSpecification<UserStub>(v => inputDateTimes.Contains(v.ModifyDate));
            string actualSql = GenerateSql(specification);

            Assert.AreEqual($"ModifyDate IN ({GetOracleDateTimeFormat(dateTime1)}, {GetOracleDateTimeFormat(dateTime2)}, {GetOracleDateTimeFormat(dateTime3)})", actualSql);
        }
        ...
        public void Generate_GenerateFromNotIn_ShouldEqualsSqlResult()
        {
            ...
            Assert.AreEqual($"ModifyDate NOT IN ({GetOracleDateTimeFormat(dateTime1)}, {GetOracleDateTimeFormat(dateTime2)}, {GetOracleDateTimeFormat(dateTime3)})", actualSql);
        }
        ...
        {
            var specification = new AnonymousSpecification<UserStub>(v => v.ModifyDate.Equals(_expectedDateTime));
            string actualSql = GenerateSql(specification);
        ...
        public void Generate_GenerateFromNotEqualsValueMethodCall_ShouldEqualsSqlResult()
        {
            ...
            Assert.AreEqual($"NOT ((ModifyDate = {_expectedOracleDateTime}))", actualSql);
        }
    }
}
